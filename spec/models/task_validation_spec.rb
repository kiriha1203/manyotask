...
RSpec.describe 'タスク管理機能', type: :model do
  ...
  it 'end_deadlineが空ならバリデーションが通らない' do
    task = Task.new(name: '失敗テスト', content: '失敗テスト', end_deadline: '', priority: "高", status: "完了")
    expect(task).not_to be_valid
  end
  it 'priorityが空ならバリデーションが通らない' do
    task = Task.new(name: '失敗テスト', content: '失敗テスト', end_deadline: '2030-05-30', priority: "", status: "完了")
    expect(task).not_to be_valid
  end
  it 'statusが空ならバリデーションが通らない' do
    task = Task.new(name: '失敗テスト', content: '失敗テスト', end_deadline: '2030-05-30', priority: "高", status: "")
    expect(task).not_to be_valid
  end
